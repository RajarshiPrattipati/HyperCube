...
poll_data = {
       'question' : 'Is it time for the class party yet? :',
    }
...
@app.route('/poll')
def poll():
    id = int(request.args.get('vid'))
    submitVote(id,request.args.get('str1'),request.args.get('str2'),request.args.get('str3'),request.args.get('str4'),request.args.get('str5'))
    print(id)
    return render_template('thanks.html', data=poll_data)



def submitVote(i,a,b,c,d,e):

    out = open(filename, 'a')
    out.write(str(i) + ' ' + str(a) + ' ' + str(b) + ' ' + str(c) + ' ' + str(d)  + ' ' + str(e) +'\n' )
    out.close()
...
class InteractiveCube(plt.Axes):
 ...
 #       self._btn_solve.on_clicked(self._solve_cube)
        ...
        self._ax_vote = self.figure.add_axes([0.55, 0.05, 0.2, 0.075])
        self._btn_vote = widgets.Button(self._ax_vote, 'Finalize votes')
        self._btn_vote.on_clicked(tally)
...
def int_to_rotation(argument):
    ...
    func = switcher.get(argument, lambda: "InvalidRotation")
    func()
...
def tally(q):	
	...
	for v in range(NUM_USERS-1,-1,-1):
		l = votes[str(v+1)]
		for x in l[::-1]:
			int_to_rotation(int(x)+6)		
...
class Voter:
    def __init__(self, id, turns ):
        ...
        for i in range(turns):
            self.M.append(randint(1,6))
	mail = raw_input("Voter number "+ str(id)+ " Enter Email Address : ")
	sendMail(mail,self.M)
	

def sendMail(mail,M):
	serve = smtplib.SMTP('smtp.gmail.com', 587)
	serve.starttls()
	serve.login("pollappemail", "poller123")
	serve.sendmail("pollappemail@gmail.com", str(mail), str(M))  
